...
class IndicateFalsePositives: public osmium::handler::Handler {
    ...
    location_handler_type &location_handler;bool analyse_ways = true;
    ...
    bool is_valid(const osmium::OSMObject& osm_obj) {
        ...
        if (!analyse_ways) {
            ...
            if ((water)
                    && ((!strcmp(water, "river")) || (!strcmp(water, "drain"))
                            || (!strcmp(water, "stream"))
                            || (!strcmp(water, "canal"))
                            || (!strcmp(water, "ditch")))) {
                return false;
        ...
        if (multipolygon) {
            ...
            if (results.size()) {
                for (auto result : results) {
                    ...
                    try {
                        node_id =
                                *(static_cast<osmium::object_id_type*>(result));
                    } catch (...) {
                        continue;
                    }
                    ...
                    if (contains) {
                        ...
                        if (error_node != ds.error_map.end()) {
                            ...
                            cerr
                                    << "Enexpected error: error_tree contains node, but not error_map."
                                    << endl;
                            exit(1);
                        }
                        { //Benchmark
...
//        if (area.get_value_by_key("waterway")) {
//            cout << area.get_value_by_key("waterway") << endl;
//        }
